import { Modal, Group, Stack, Text, Paper, SimpleGrid, Title, Table } from '@mantine/core';
import { Game } from '../types';
import { IconCards, IconLayoutBoard } from '@tabler/icons-react';
...
export function KeyStatsModal({ game, opened, onClose }: KeyStatsModalProps) {
  ...
  const totalCardCopies = game.decks.reduce(
    (sum, deck) => sum + deck.cards.reduce((cSum, card) => cSum + (card.count || 1), 0),
    0
  );
  const totalFronts = game.decks.reduce(
    (sum, deck) => sum + Object.keys(deck.frontStyles).length,
    0
  );
            ...
            <Table.Tbody>
              {game.decks.map((deck) => (
                <Table.Tr key={deck.id}>
                  ...
                  <Table.Td>
                    {deck.cards.length} ({deck.cards.reduce((s, c) => s + (c.count || 1), 0)}{' '}
                    copies)
                  </Table.Td>
