...
export function StyleEditor({ deck: externalDeck, setDeck: setExternalDeck }: StyleEditorProps) {
  ...
  const addElement = (type: 'text' | 'image' | 'shape', shapePreset?: string) => {
    ...
    const newElement: LayoutElement = {
      id: `el-${Date.now()}`,
      name:
        type === 'shape' && shapePreset ? shapePreset : type === 'text' ? 'New Text' : 'New Image',
      type,
      x: 10, // mm
      y: 10, // mm
  ...
  const duplicateElement = (elementId: string, targetStyleIds: string[]) => {
    if (!selectedStyleId) return;
    const elToCopy =
      deck.frontStyles[selectedStyleId]?.elements.find((e: LayoutElement) => e.id === elementId) ||
    ...
    notifications.show({
      title: 'Success',
      message: `Copied element to ${targetStyleIds.length} styles`,
    });
  };
  ...
  const renameStyleId = (newId: string) => {
    ...
    if (!/^[a-z0-9-_]+$/.test(newId)) {
      notifications.show({
        title: 'Error',
        message: 'ID can only contain lowercase letters, numbers, dashes and underscores.',
        color: 'red',
      });
      return;
    }
    ...
    if (styles[newId]) {
      notifications.show({
        title: 'Error',
        message: 'Style with this ID already exists.',
        color: 'red',
      });
      return;
    }
        ...
        <Stack w={300} h="100%" style={{ borderRight: '1px solid #eee' }} p="md">
          ...
          <Group>
            <Select
              ...
              style={{ flex: 1 }}
            />
            <ActionIcon variant="light" onClick={addNewStyle} mt={25}>
              <IconPlus size={16} />
            </ActionIcon>
          </Group>
          ...
          <Group>
            <Button size="xs" onClick={() => addElement('text')}>
              Add Text
            </Button>
            <Button size="xs" onClick={() => addElement('image')}>
              Add Image
            </Button>
            <Menu shadow="md" width={200}>
              <Menu.Target>
                <Button size="xs" rightSection={<IconPolygon size={14} />}>
                  Add Shape
                </Button>
              </Menu.Target>
              <Menu.Dropdown>
                  ...
                  currentStyle.elements.map((el) => {
                    ...
                    return (
                      <Rnd
                        ...
                        style={{
                          border:
                            selectedElementId === el.id ? '1px solid #228be6' : '1px dashed #ccc',
                          cursor: 'move',
                      ...
                      >
                        ...
                        >
                          {el.type === 'image' ? (
                            ...
                            >
                              <polygon
                                points={(tempPoints && tempPoints.id === el.id
                                  ? tempPoints.points
                                  : el.points
                                )
                              ...
                              {selectedElementId === el.id &&
                                (tempPoints && tempPoints.id === el.id
                                  ? tempPoints.points
                                  : el.points
                                ).map((p, i) => (
                                  <circle
